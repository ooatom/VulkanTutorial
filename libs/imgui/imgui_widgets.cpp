...
// Tip: pass a non-visible label (e.g. "##hello") then you can use the space to render other text or image.
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    // Draw separator
    // (it would be misleading to render this in EndTabBar() suggesting that it may be drawn over tabs, as tab bar are appendable)
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a texture), so need to add an extra render call (temporary in the case of vertical animation)
    ...
    // Restore main window position so user can render there
    if (want_clip_rect)
        PopClipRect();
    window->DC.CursorPos = backup_main_cursor_pos;
