...
//    | TableDrawContextMenu()                  - render right-click context menu
//-----------------------------------------------------------------------------
...
//    | TableDrawBorders()                      - render outer borders, inner vertical borders
//    | TableMergeDrawChannels()                - merge render channels if clipping isn't required
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // limits of that rectangle, in order for ImDrawListSplitter::Merge() to merge the render commands. However since the overlap
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // - ClipRect.Min.x: Because merging render commands doesn't compare min boundaries, we make ClipRect.Min.x match left bounds to be consistent regardless of merging.
    ...
    // [Part 9] Allocate render channels and setup background cliprect
...
void    ImGui::EndTable()
{
    ...
    // Pros: remove render calls which will have no effect. since they'll have zero-size cliprect they may be early out anyway.
    ...
    // Flatten channels and merge render calls
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ...
    // We cannot render neither the cell or row background immediately as we don't know the row height at this point in time.
    switch (target)
    {
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    {
        // FIXME-TABLE: Could avoid this if render channel is dummy channel?
...
// Allocate render channels. Called by TableUpdateLayout()
// - We allocate them following storage order instead of display order so reordering columns won't needlessly
//   increase overall dormant memory cost.
// - We isolate headers render commands in their own channels instead of just altering clip rects.
//   This is in order to facilitate merging of render commands.
// - After crossing FreezeRowsCount, all columns see their current render channel changed to a second set of channels.
// - We only use the dummy render channel so we can push a null clipping rectangle into it without affecting other
//   channels, while simplifying per-row/per-cell overhead. It will be empty and discarded when merged.
// - We allocate 1 or 2 background render channels. This is because we know TablePushBackgroundChannel() is only used for
//   horizontal spanning. If we allowed vertical spanning we'd need one background render channel per merge group (1-4).
// Draw channel allocation (before merging):
// - NoClip                       --> 2+D+1 channels: bg0/1 + bg2 + foreground (same clip rect == always 1 render call)
// - Clip                         --> 2+D+N channels
...
void ImGui::TableSetupDrawChannels(ImGuiTable* table)
{
    ...
    // Initial draw cmd starts with a BgClipRect that matches the one of its host, to facilitate merge render commands by default.
    // All our cell highlight are manually clipped with BgClipRect. When unfreezing it will be made smaller to fit scrolling rect.
    // (This technically isn't part of setting up render channels, but is reasonably related to be done here)
...
// This function reorder render channels based on matching clip rectangle, to facilitate merging them. Called by EndTable().
...
// We reorder render commands by arranging them into a maximum of 4 distinct groups:
//
...
// This function assume that each column are pointing to a distinct render channel,
...
// - The channel uses more than one render command itself. We drop all our attempt at merging stuff here..
//   we could do better but it's going to be rare and probably not worth the hassle.
// Columns for which the render channel(s) haven't been merged with other will use their own ImDrawCmd.
//
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        for (int merge_group_sub_n = 0; merge_group_sub_n < merge_group_sub_count; merge_group_sub_n++)
        {
            ...
            // Don't attempt to merge if there are multiple render calls within the column
        ...
        // Invalidate current render channel
    ...
    if (merge_group_mask != 0)
    {
        ...
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                // Extend outer-most clip limits to match those of host, so render calls can be merged even if
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            // Always render full height border when being resized/hovered, or on the delimitation of frozen column scrolling.
    ...
    {
        // Display outer border offset by 1 which is a simple way to display it without adding an extra render call
...
// We cpu-clip text here so that all columns headers can be merged into a same render call.
...
void ImGui::TableHeader(const char* label)
{
    ...
    // be merged into a single render call.
...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    // Declare row, override and render our own background
    TableNextRow(ImGuiTableRowFlags_Headers, row_height);
    TableNextColumn();
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                // Draw label (first render at an offset where RenderTextXXX() function won't meddle with applying current ClipRect, then transform to final offset)
...
// Get into the columns background render command (which is generally the same draw command as before we called BeginColumns)
